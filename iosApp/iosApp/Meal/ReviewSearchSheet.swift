...
/// Search sheet used inside the meal review flow to add an item from the catalog.
/// Produces a `PhotoMealEventAddSearchItem` event via `onSelect`.
struct ReviewSearchSheet: View {
    ...
    private var emptyQueryPrompt: some View {
        VStack(spacing: 12) {
            ...
            Text("Search the catalog")
                .font(.strakkHeading3)
                .foregroundStyle(Color.strakkTextSecondary)
            Text("Type a food name to add it to the meal.")
                .font(.strakkBody)
                .foregroundStyle(Color.strakkTextTertiary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 40)
